...
#if defined(NDEBUG)
#error "InstaDEX cannot be compiled without assertions."
#endif
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60) && chainActive.Height() > 26550; // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
{
    RenameThread("instadex-scriptch");
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    // TODO : ENABLE INDEX CACHE IN SPECIFIC CASES
