...
bool Session::Impl::processTransactionHold(XBridgePacketPtr packet) const
{
...

    {
        ...
        // check masternode
        CMasternode * snode = mnodeman.Find(pksnode);
        if (!snode)
        {
            ...
            if (!snode)
            {
                // bad masternode, no more
                LOG() << "unknown masternode " << pksnode.GetID().ToString() << " " << __FUNCTION__;
                return true;
            }
        }
    }
    ...
    LOG() << "use masternode " << pksnode.GetID().ToString() << " " << __FUNCTION__;
...
bool Session::Impl::processTransactionInit(XBridgePacketPtr packet) const
{
    ...
    std::vector<unsigned char> snodeAddress;
    {
        ...
        LOG() << "use masternode " << id.ToString() << " " << __FUNCTION__;
    }
