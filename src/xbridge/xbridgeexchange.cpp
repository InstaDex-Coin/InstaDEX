...
bool Exchange::init()
{
    ...
    if (!m_p->initKeyPair())
    {
        ERR() << "bad masternode key pair " << __FUNCTION__;
    }
...
{
    return ((m_p->m_wallets.size() > 0) && GetBoolArg("-masternode", false));
}
...
{
    return (isEnabled() && (activeMasternode.status == ACTIVE_MASTERNODE_STARTED));
}
...
{
    std::string secret = GetArg("-masternodeprivkey", "");
    if (secret.empty())
    {
        ERR() << "masternode key not set " << __FUNCTION__;
        return false;
    }
    ...
    {
        ERR() << "invalid masternode key " << __FUNCTION__;
        return false;
    }
