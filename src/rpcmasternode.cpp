...
Value obfuscation(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <instadexaddress> <amount>\n"
            "instadexaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <instadexaddress> <amount>\n"
            "instadexaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in instadex.conf\n"
            ...
            if (mne.getAlias() == alias) {
                ...
                if (!snodeTx.IsNull() && mne.castOutputIndex(snodeTxIdx) && static_cast<int>(snodeTx.vout.size()) > snodeTxIdx) {
                    ...
                    if (snodeTxAmount != MASTERNODE_REQUIRED_AMOUNT*COIN) {
                        ...
                        statusObj.push_back(Pair("errorMessage", strprintf("Masternode input requires %d INDEX. Only %f INDEX was found",
    ...
    if (strCommand == "start-many" || strCommand == "start-all" || strCommand == "start-missing" || strCommand == "start-disabled") {
        ...
        BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            if (success) {
                ...
                if (snodeTxAmount != MASTERNODE_REQUIRED_AMOUNT * COIN) {
                    success = false;
                    errorMessage = strprintf("Masternode input requires %d INDEX. Only %f INDEX was found",
                                                       MASTERNODE_REQUIRED_AMOUNT, (float) snodeTxAmount / (float) COIN);
                }
...
Value masternodelist(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1)) {
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",         (string) Masternode InstaDEX address\n"
