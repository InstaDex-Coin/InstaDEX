...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("InstaDEXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("InstaDEXMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("InstaDEXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("instadex-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running InstaDEXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
