...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::INDEX)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("INDEX")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeInstaDEXAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeInstaDEXAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeInstaDEXAmount = strAnonymizeInstaDEXAmount.remove(strAnonymizeInstaDEXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeInstaDEXAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeInstaDEXAmount * COIN) nMaxToAnonymize = nAnonymizeInstaDEXAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeInstaDEXAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeInstaDEXAmount));
        strAnonymizeInstaDEXAmount = strAnonymizeInstaDEXAmount.remove(strAnonymizeInstaDEXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeInstaDEXAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
