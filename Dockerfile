...
# docker build --build-arg cores=4 -t instadex/devbuilds:3.9.10 .
FROM ubuntu:trusty
...
# Copy source files
COPY . /opt/instadex/BlockDX/

# Build source
RUN mkdir -p /opt/instadex/BlockDX \
  && mkdir -p /opt/blockchain/config \
  && mkdir -p /opt/blockchain/data \
  && ln -s /opt/blockchain/config /root/.instadex \
  && cd /opt/instadex/BlockDX \
  ...
  && make -j$ecores \
  && make install \
  && rm -rf /opt/instadex/ /tmp/berkeley/*

# Write default instadex.conf (can be overridden on commandline)
...
rpcclienttimeout=15                       \n\
rpcuser=test                              \n\
rpcpassword=user" > /opt/blockchain/config/instadex.conf
